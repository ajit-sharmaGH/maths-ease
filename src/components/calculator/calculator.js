...
const Calculator = () => {
  ...
  const [total, setTotal] = useState(numberOne + numberTwo);
  const addNumbers = () => {
    setTotal(
      Number(numberOne) + Number(numberTwo)
    ); /**by default it behaves like string so added Number**/
  };
  const subNumbers = () => {
    ...
    <div className="bg-theme-changer calculator_body">
      <div className="pt-2">
        ...
        {numberTwo.length > 0 && numberOne.length > 0 ? (
          <h2 className="flex-center">{total}</h2>
        ) : (
